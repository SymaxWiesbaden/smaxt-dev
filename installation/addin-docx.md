## Installing the smaxt Word-Add-In

With the smaxt Wordadd-in, you can create interactive layouts for DOCX documents to be created in a database. You can easily insert variables and placeholders into a Word document and add formulas or diagrams.

### System Requirements

The smaxt Word add-in requires Microsoft Word™ 2010 or higher.
...
Download the setup program from www.smaxt.com `Setup_smaxt_Word_Add-In_V3.0_en.zip` or detach the installer from the e-mail you received together with the license information. Copy the file to a directory on the target machine.

#### 2. Unzip the archive

Open a command line and unzip the archive with the command:

`Unzip setup_smaxt_Word_Add-In_V3.0_en.zip`

#### 3. Install the Add-In

First quit all currently running instances of Word. Then run the setup-routine from the folder where you unzipped the archive and follow the on-screen instructions.
...
After successful installation Word starts with an additional ribbon entry labeled "smaxt". The smaxt-specific operations can now be carried out within Word. For a first introduction to this add-in, see chapter "Getting Started".
